...
//We store our args as a list of cons types where the first cons type points to the list of list of bindings ((x 3)) and every following cons type
    ...
    //Create a new frame and call it oldframe that will refer to the current frame (not newframe)
	Frame *newFrame;
	...
	while (binders->type != NULL_TYPE){
        ...
        //Create a new frame that has parent pointing to oldrame(current) frame and no bindings.
	    newFrame = talloc(sizeof(Frame));
	    newFrame->bindings = makeNull();
	    newFrame->parent = oldFrame;
		Value *newBinding = makeNull();
		Value *symbol = car(car(binders));
		// Creating binders of newFrame but to make it letstar, evaluate it in newframe's parent frame so it can't recursively call itself.
...
//We store our args as a list of cons types where the first cons type points to the list of list of bindings ((x 3)) and every following cons type
	...
	while (binders->type != NULL_TYPE){
		...
		Value *newBinding = makeNull();
		Value *symbol = car(car(binders));
		// Creating binders of newFrame but evaluate it in newframe so it can call itself recursively.
...
Value *evalSetBang(Value *args, Frame *frame){
    ...
    //Goes through all existing bindings recursively(recurses to the parent frame when the binding that needs to be updated isn't found in the current frame.)
    Value *bindings = frame->bindings;
    while (bindings->type != NULL_TYPE){
        if (!strcmp(car(args)->s, car(car(bindings))->s)){
            Value *result = eval(car(cdr(args)), frame);
            //Then changes the value of the binding after it finds the binding.
...
Value *evalAnd(Value *args, Frame *frame) {
    //evaluates and by creating a bool type and if any of the arguements are false, it returns false. It also shortcircuits if the arguement is false. 
    if (length(args) < 2) {
...
Value *evalOr(Value *args, Frame *frame){
    //evaluates or by creating a false bool type and if any of the arguments are true, change the return value to be true. Also shortciruits if any of the arguements are true.
...
//the evaluation of the cond special form
Value *evalCond(Value *args, Frame *frame){
    Value *toReturn = talloc(sizeof(Value));
    while (cdr(args)->type != NULL_TYPE){
        Value *cond_clause = car(args);
        //return eval(car(cdr(cond_clause)), frame) which is just evaluating each cond arguement
    ...
    // the last case is special because it can be else but otherwise we evaluate it normally
    Value *cond_clause = car(args);
    ...
    //if the cond is empty we return the void type
    else{
        Value *boolean = eval(car(cond_clause), frame);
        if (boolean->i == 0){
            toReturn->type = VOID_TYPE;
